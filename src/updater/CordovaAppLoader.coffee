...
module.exports = CordovaAppLoader = (options) ->
  ...
  console.log "sdf", @_checkTimeout


CordovaAppLoader::_createFilemap = (files) ->
  result = {}
...
CordovaAppLoader::copyFromBundle = (file) ->
  url = BUNDLE_ROOT + file
  @cache._fs.download url, @cache.localRoot + file

CordovaAppLoader::getBundledManifest = ->
...
CordovaAppLoader::check = (newManifest) ->
  self = this
  manifest = @manifest
...
CordovaAppLoader::canDownload = -> !!@newManifest and !@_updateReady


CordovaAppLoader::canUpdate = -> @_updateReady


CordovaAppLoader::download = (onprogress) ->
...
CordovaAppLoader::update = (reload) ->
  if @_updateReady
    # update manifest
...
CordovaAppLoader::clear = ->
  localStorage.removeItem "last_update_files"
  localStorage.removeItem "manifest"
  @cache.clear()


CordovaAppLoader::reset = ->
  @clear().then (-> location.reload()), -> location.reload()